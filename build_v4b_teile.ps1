# ============================================================
#  v4b — korrigierter Teilesatz fuer das Kontakt-Mikrometer (autonom).
...
foreach($sy in -1,1){                                  # glatte Ø8-Wellen: Presssitz (+Kleber)
...
foreach($a in 45,135,225,315){                          # 4x M3-Pilots auf Ø16-Lochkreis
  $r=[math]::PI*$a/180.0
...
$s=$cd.Sketches.Add($XY); Rect $s -14 -8 14 8
$ef.AddByDistanceExtent($s.Profiles.AddForSolid(), 24*$f, $kPos, $kJoin)|Out-Null
$s=$cd.Sketches.Add($XY); Circ $s 0 0 8.0              # Ø8-Zapfen nach unten
